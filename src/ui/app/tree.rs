...
impl App {
    pub(super) fn render_tree(&mut self, f: &mut Frame, area: Rect) {
        ...
        if self.search_query.is_some() {
            ...
            self.render_search_results(f, search_area, left_focus);
        } else {
            self.render_tree_normal(f, inner, left_focus);
        }
    }
    ...
    fn render_tree_normal(&mut self, f: &mut Frame, inner: Rect, left_focus: bool) {
        let (gid, pid) = self.connected_id().unwrap_or((-1, -1));
        let mut items: Vec<ListItem> = Vec::new();
        let mut cursor_pos_in_list = 0;
        let mut pos = 0;
